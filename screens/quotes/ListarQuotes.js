...
import CardQuotes from '../../components/CardQuotesComponent'; // Path to your CardQuotesComponent
import {
    listarQuotes,
    eliminarQuotes,
...
export default function ListarQuotes() {
    ...
    const fetchQuotes = useCallback(async () => {
        setLoading(true);
        try {
            const result = await listarQuotes();
            if (result.success) {
        ...
        Alert.alert(
            ...
            [
                { text: 'Cancelar', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            const result = await eliminarQuotes(id);
                            if (result.success) {
                                Alert.alert('Éxito', 'Usuario EPS eliminado correctamente.');
                                fetchQuotes(); // Reload the list after deletion
                            } else {
    ...
    const handleEditarQuotes = (quotesItem) => {
        // Navigates to the 'EditarQuotes' screen, passing the user EPS data
        navigation.navigate('EditarQuotes', { quotes: quotesItem });
    };

    const handleCrearQuotes = useCallback(() => {
        // Navigates to the 'EditarQuotes' screen without data for creation
        navigation.navigate('EditarQuotes');
    }, [navigation]);

    const handleDetailQuotes = useCallback(
        (quotesItem) => {
            // Navigates to a 'DetalleQuotes' screen to show more details
            // Make sure you have a 'DetalleQuotes' route configured in your navigator.
            navigation.navigate('DetalleQuotes', { quotes: quotesItem });
        },
        [navigation]
    );
        ...
        <View style={styles.container}>
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <CardQuotes
                        quotes={item}
                        onDetail={() => handleDetailQuotes(item)}
                        onEdit={() => handleEditarQuotes(item)}
                        onDelete={() => handleEliminarQuotes(item.id)}
                        cardStyle={styles.quotesCard}
                        textStyle={styles.quotesCardText}
                    />
                )}
                ListEmptyComponent={
